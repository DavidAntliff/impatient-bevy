...
// Spritesheet atlas constants
const TILE_SIZE: u32 = 64; // 64x64 pixel tiles
...
fn spawn_player(
    ...
    // Load the spritesheet and build a grid layout
    let texture_handle = asset_server.load("skeleton_spritesheet.png");
...
fn animate_player(
    ...
    // Compute the target row and position in the atlas
    ...
    if anim.moving {
        if just_started {
            ...
            // Restart the timer so the next frame lasts a full interval
            timer.reset();
        } else {
            // Continuous movement based on timer cadence
        ...
        // Return to idle frame, reset timer for transition to idle
        let row_start = row_start_index(anim.facing);
        atlas.index = row_start + 0;

        // Or, hold the current frame:
        //timer.reset();
    }
